...
/**
 * The app serves static files out of `public/` — currently the favicons.
 * Nothing holds a link and its file together: rename or drop one of them and
 * the build passes, ESLint passes, every type check passes, and the reference
 * resolves to nothing at runtime.
 *
 * The set is small today, which is exactly when this is cheap to install. It
 * earns its place the first time a guide PDF is added here and later moved or
 * renamed — which is how the sibling repos acquired the same guard.
 *
 * This walks the source for root-relative asset references and asserts each one
 ...
 * PDFs are served from spertsuite.com — are out of scope, because asserting on
 * them here would make this suite depend on another repository's deploy.
 */
