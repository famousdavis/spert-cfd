...
- **The safety check on the mutation-testing run had a hole shaped like the thing it guards against.** That check exists to refuse a run that produced no real results but reports a clean-looking one. It counted "no test reaches this code" as a real result. So a run in which every single change went untested — nothing checked, a score of zero per cent — was reported as "the run produced real verdicts". A tool certifying the exact failure it was written to catch.
- **The fix is one line, and it separates two questions that had been sharing one sum.** Whether a change was left untested is a real fact about the code, and it still counts towards the score exactly as before. Whether the test suite ever actually ran is a different question, and an untested change is silence rather than evidence. The check now asks only the second.
- **The cause originally recorded for this was wrong, and that is worth more than the fix.** The suite's own notes said the all-untested state came from one specific misconfiguration. Five attempts were made to produce that state on purpose, in two projects, including the one where the misconfiguration should bite hardest. None of them produced it: the tool either refuses to start when no test covers the changed code, or runs the whole suite and reports the changes as surviving instead. The note was describing behaviour the tools no longer have.
- **So the new note describes the state being refused and names no cause for it.** A note that explains a fault by pointing at one trigger stops being true when the trigger changes; a note that describes what is being refused does not. The hole was real regardless of how a run reaches it, and the fix is unaffected.
- **What was verified, and what was not.** Four hand-built cases were run through the real checking script, both before and after the change: the faulty case now fails and named its reason, the ordinary case with some untested code still passes, and two already-failing cases are unchanged. Every existing measurement in the projects that keep one still passes, with an identical score. Two of the four projects have a stored measurement containing untested code, which is the case the change had to leave alone; one has none and one has no stored measurement at all, so they are not evidence.

## v0.15.7 — Mutation testing establishes a first quality baseline (August 22, 2026)
...
- **The note at the top of the shared release-checking script was false, and had been from the day it was written.** That script is deliberately the same file in all nine SPERT® Suite projects. Its opening note said there was no automated checking anywhere in the suite — that a green tick on a proposed change meant only that a preview copy had been built, and that nothing ran the tests. Automated checking has run on every one of the nine projects since the script existed, on every proposed change and on every merge, and what it runs is this very script. It gated all four of this project's releases this week.
- **It did not go out of date — it was untrue at the moment it was written.** The same set of edits that added the script also switched the automated checking on, so the file contradicted a change sitting beside it. That distinction decides the remedy, which is why it is recorded here: a statement that decays can be helped by writing down when it was made; a statement that was never true cannot. What went wrong was that a claim about the projects was written into an explanation without being checked against them, and an explanation is read as background rather than as an assertion somebody has to verify.
- **The damage was quiet.** The note told a reader that a green check means nothing, so anyone reading it would discount a real signal or repeat work already checked — a correct-looking pause resting on a false premise, producing no error and simply spending a round trip.
- **Two explanations were added to the same file.** The first records that automated checking and a check run by hand are complementary rather than ranked: the automated one works from a clean copy and catches anything that quietly depends on a file existing only on the author's machine, but it also has less of the project to look at, so certain checks step aside there and only a hand-run finds what those cover.
- **The second explains the accepted-issue-count trade this project adopted one release ago.** The code-style step compares the number of reported issues against an agreed figure instead of reading pass or fail, and it does so for opposite reasons across the suite: where the step reports failure at the agreed figure, reading pass-or-fail would be too strict; where the findings are all advisories and the step reports success, reading pass-or-fail would be too lenient and would let new issues through unnoticed. The note also records that the figure counts every kind of issue rather than the one kind a project set it for, and that when it reaches zero the setting must be removed rather than set to zero — at zero the tool prints no count at all, and the step then fails asking for a number that was never printed. Those three points were written into this project's own configuration in v0.15.5; they now live in the shared file, where every project can read them.
- **A reference in this project stopped pointing at line numbers.** The v0.15.5 note cited a line range inside the shared script. Adding lines to the top of that script moved it, one day after it was written. It now names the part of the script it means. A stale line number is worse than a missing one: it lands on real code, so a reader who follows it finds something plausible and concludes the reference was sound.
- **No drift check is proposed as the remedy, and that is deliberate.** The file was identical in all nine projects the entire time the claim was false, so a check comparing the copies to each other would have passed on every run and licensed the conclusion that the file was in order. A guard that measures agreement between copies cannot detect a fault all the copies share.
...
### Changed
- Removed this repository's local copy of `firestore.rules`. Firestore security rules are deployed from the Firebase Console and mirrored in the SPERT® Suite landing-page repository, which is their single source of truth. The copy kept here was never deployed from and could only drift out of date; it was never bundled into the app, so cloud behaviour is unchanged. (chore)
- Resynchronised the version surfaces: `package-lock.json` had been left at 0.14.6 while `package.json` read 0.14.7. Both now read 0.14.8. (chore)
